...
from src.Application.decrypt_timing import _copy_timing
from src.Infrastructure.runtime_logging import timing_text
    ...
    try:
        if should_transcode and _artifact_needs_transcode(prepared.desired_target, prepared.detected_container):
            working_path, final_extension, transcode_meta = _maybe_transcode(
                ...
                prepared.detected_container, prepared.file_timing,
        ...
        logger.info("[timing] file_done [%d/%d] %s reason=%s %s",
                    prepared.index, prepared.total_count, prepared.input_path.name, exc,
                    timing_text(prepared.file_timing))
        result = FileResult(
