"""Application.decrypt_service — batch decryption orchestration.

Infrastructure dependencies (RuntimePaths, CoverArtService, ...) are injected
via optional ``ports`` argument for new Presentation-layer callers. Legacy
callers that omit ``ports`` fall back to ``Infrastructure.di.build_default_ports``.
"""
...
from src.Application.decrypt.decrypt_timing import (
...
from src.Application.decrypt.decrypt_orchestrate import (
...
def _lazy_setup_ports(ports: ApplicationPorts | None) -> ApplicationPorts:
    """Build default ports bundle via Infrastructure DI (backward-compat)."""
    if ports is not None:
        return ports
    from src.Infrastructure.di import build_default_ports
    return build_default_ports()

def run_batch(config: BatchRunConfig, adapter: PlatformAdapter, ports: ApplicationPorts | None = None) -> int:
    ...
    for index, file_path in enumerate(files, start=1):
        ...
        try:
            ...
            decrypt_summary = _log_media_summary(logger, "Decrypt media summary", working_path, transcode_port)
            ...
            logger.info("decoded_ready: %s container=%s target=%s needs_transcode=%s",
                        ...
                        _artifact_needs_transcode(normalized_target, detected_container, transcode_port))
            _emit_event(config, "file_decrypted", {
                ...
                "needs_transcode": _artifact_needs_transcode(normalized_target, detected_container, transcode_port),
