...
from src.Application.services.config_service import (
    ...
    config_service, default_kuwo_signature_path, format_help_epilog,
...
def _run_transcode_batch_cli(paths: RuntimePort, config: dict[str, Any], args: argparse.Namespace) -> int:
...
def _run_kugou_refresh_key_cli(paths: RuntimePort, config: dict[str, Any], args: argparse.Namespace) -> int:
    ...
    if args.output:
        ...
        output_path = kugou_service.default_refreshed_key_path(paths)
    try:
        result = kugou_service.refresh_key(paths, destination=output_path)
...
def build_parser(paths: RuntimePort) -> argparse.ArgumentParser:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.platform is None:
        ...
        if admin_code is not None:
            return admin_code
        return run_interactive()
    _, config = config_service.load(paths)
