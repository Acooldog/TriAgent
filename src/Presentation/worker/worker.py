...
from src.Presentation.worker.worker_runtime import PROTOCOL_VERSION, WorkerRequestError, WorkerRuntime
...
def run_decrypt(payload: dict[str, Any], runtime: WorkerRuntime) -> int:
    ...
    with contextlib.redirect_stdout(sys.stderr):
        result_code = run_batch(config, create_platform_adapter(platform_id))
...
def run_agent(payload: dict[str, Any], runtime: WorkerRuntime) -> int:
    ...
    from src.Application.services.agent_service import agent_service
    runtime.emit("agent_message", payload={
        "content": agent_service.build_initial_action_message(user_message),
        "kind": "progress",
    })
    runtime.log("正在导入 agent_service...")
    _run_agent = agent_service.run_agent
    check_langchain_available = agent_service.check_langchain_available
    runtime.log("agent_executor 导入完成")
    try:
        agent_service.set_ask_user_callback(runtime.ask_user)
        perm_mode = str(payload.get("permission_mode", "standard") or "standard").lower()
        if perm_mode not in ("restricted", "standard", "full"):
            perm_mode = "standard"
        agent_service.set_permission_mode(perm_mode)
