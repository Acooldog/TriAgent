...
- **禁止用 `run_cli_safely` 执行 ffmpeg 转码**：格式转换必须使用 `transcode_audio` 或 `process_audio` 工具
...
## ⛔ 停止规则（重要！）
- 当所有平台的解密+转换都已完成且结果报告完毕，**立即停止调用工具**，直接向用户报告任务完成
- **不要**在任务完成后调用 verify_audio_integrity、list_directory 等额外工具做"检查"或"验证"
- 如果所有 decrypt 工具返回"全部成功"，直接汇总结果并结束
- 如果只有少量失败，报告失败详情后结束，不要为了修复少量失败反复尝试
- 简单说：做完就停，不要画蛇添足
