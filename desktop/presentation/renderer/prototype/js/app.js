...
import { createDomEnhancers, createTypewriter } from "./domEnhancers.js";
...
function replyFor(prompt) {
  if (prompt.trim() === "1") return "你发送了\"1\"，我可以先陪你做一次完整的 TriMusicAgent 流程测试。你可以继续输入音乐文件路径、想解密的平台、输出格式，或者直接描述你想得到的结果。我会把用户消息放在右侧，把 TriMusicAgent 的回复放在左侧，并持续把新消息追加到滚动容器底部。\n\n你也可以测试三种模式：受限模式会先提示敏感操作，标准模式会在需要时请求确认，完全访问模式会自动执行允许范围内的任务。联网检索模式会在消息流中插入状态分隔线，方便你确认当前上下文。";
  return `我收到你的需求：\"${prompt}\"。这是一个未接入真实模型的原型回复。你可以继续补充文件、路径、平台、输出格式或错误信息，我会把每次输入追加为新的消息，并模拟 TriMusicAgent 的处理过程。`;
}
...
function startLlmResponse() {
  ...
  streamText(fullText, {
    ...
    delay: state.llmOutputSpeed,
    onChunk: (index) => { if (state.llmStreaming) { state.llmStreaming.index = index; updateLlmStreamDom(index); } },
    onDone: () => { state.llmMessages = [...state.llmMessages, { role: "assistant", text: fullText }]; state.llmStreaming = null; llmStreamSession = null; if (state.contextUsage >= state.compressionThreshold) compressLlmContext(); render(); },
    onAbort: (index) => { state.llmMessages = [...state.llmMessages, { role: "assistant", text: fullText.slice(0, index) || "TriMusicAgent 已被打断。" }]; state.llmStreaming = null; llmStreamSession = null; render(); }
  });
}

function startLlmRetry() {
  llmStreamSession?.abort();
  llmStreamSession = createStreamSession();
  state.llmRetry = { attempt: 0, max: 5 };
  render();
  retryConnection(async () => { throw new Error("unexpected status 403 Forbidden：模型连接失败或余额不足（request id: prototype-403）"); }, {
    session: llmStreamSession,
    maxAttempts: 5,
    onAttempt: (attempt, max) => { state.llmRetry = { attempt, max }; render(); }
  }).catch((error) => {
    if (error.name === "AbortError") return;
    state.llmRetry = null;
    state.llmMessages = [...state.llmMessages, { role: "error", text: error.message }];
    llmStreamSession = null;
    render();
  });
}

function updateLlmStreamDom(index) {
  const paragraph = root.querySelector(".llm-chat-message.streaming p");
  if (paragraph && state.llmStreaming) {
    const caret = paragraph.querySelector(".streaming-caret");
    paragraph.textContent = state.llmStreaming.text.slice(0, index);
    if (caret) paragraph.append(caret);
  }
  const meter = root.querySelector(".context-meter");
  if (meter) {
    meter.style.setProperty("--usage", `${state.contextUsage}%`);
    const label = meter.querySelector("b");
    if (label) label.textContent = `${state.contextUsage}%`;
  }
  enhancers.updateBottomButton();
}

function compressLlmContext() {
  const keep = state.llmMessages.slice(-4);
  state.contextUsage = Math.max(30, Math.round(state.contextUsage * .48));
  state.llmMessages = [{ role: "notice", text: `上下文已自动压缩，保留最近对话，当前使用量 ${state.contextUsage}%` }, ...keep];
}

function interruptLlm() {
  if (!state.llmStreaming && !state.llmRetry) return;
  llmStreamSession?.abort();
  if (state.llmRetry) {
    state.llmRetry = null;
    llmStreamSession = null;
...
// Initialize DOM enhancers and action handlers
const enhancers = createDomEnhancers(root, state, { toast, render, debug });
const { sync: syncTypewriter } = createTypewriter(root, state, { toast, render });
const { handleAction } = createActionHandlers(state, root, {
  toast, render, debug, addFile, startProcessing, stopProcessing, saveToMain,
  startLlmResponse, startLlmRetry, interruptLlm,
});
...
function render() {
  ...
  enhancers.enhancePromptEditors();
  enhancers.enhancePageChrome();
  enhancers.enhanceScrollbars();
...
root.addEventListener("click", (event) => {
  ...
  if (target.dataset.page) {
    if (target.dataset.page !== state.page) state.routeHistory = [...state.routeHistory, target.dataset.page];
...
root.addEventListener("input", (event) => {
  ...
  if (target.dataset.input === "model-api-key") { state.modelConfig.apiKey = target.value; debug("model-api-key-change", state.modelConfig.apiKey); }
  if (target.dataset.input === "model-name") { state.modelConfig.model = target.value; debug("model-name-change", state.modelConfig.model); void saveToMain({ model: { defaultConfig: { ...state.modelConfig } } }); }
  if (target.dataset.input === "model-base-url") { state.modelConfig.baseUrl = target.value; debug("model-base-url-change", state.modelConfig.baseUrl); void saveToMain({ model: { defaultConfig: { ...state.modelConfig } } }); }
  if (target.dataset.input === "model-thinking") { state.modelConfig.thinking = target.value === "enabled" ? "enabled" : "disabled"; debug("model-thinking-change", state.modelConfig.thinking); void saveToMain({ model: { defaultConfig: { ...state.modelConfig } } }); }
  if (target.dataset.input === "model-max-tokens") { state.modelConfig.maxTokens = Math.max(1, Number(target.value) || 4096); debug("model-max-tokens-change", state.modelConfig.maxTokens); void saveToMain({ model: { defaultConfig: { ...state.modelConfig } } }); }
  if (target.dataset.input === "model-temperature") { state.modelConfig.temperature = Math.max(0, Math.min(2, Number(target.value) || 0.6)); debug("model-temperature-change", state.modelConfig.temperature); void saveToMain({ model: { defaultConfig: { ...state.modelConfig } } }); }
});

root.addEventListener("keydown", (event) => {
  const target = event.target;
  if (!target.matches(".llm-input, .prompt-text-input") || event.key !== "Enter" || event.shiftKey) return;
  event.preventDefault();
  handleAction(
    target.matches(".llm-input")
      ? (state.conversationMode ? "start" : state.llmStreaming ? "interrupt-llm" : "llm-send")
      : "start",
    target
  );
});
