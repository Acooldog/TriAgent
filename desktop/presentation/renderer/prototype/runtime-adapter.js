...
root?.addEventListener("click", (event) => {
  ...
  if (action === "choose-root") {
    event.preventDefault();
    event.stopImmediatePropagation();
    void chooseWorkspace().then(() => { runtime.state.workspaceRoot = "已选择工作数据根目录"; runtime.state.toast = "工作数据根目录已更新"; runtime.render(); });
  }
  if (action === "continue") {
    event.preventDefault();
    event.stopImmediatePropagation();
    void createSession().then(() => { runtime.state.page = "dashboard"; runtime.state.toast = "已创建新会话"; runtime.render(); });
  }
}, true);
...
async function startTask(runtime) {
  ...
  runtime.render();
  try {
    const result = await startAgent(prompt, runtime.state.mode, (event) => {
