...
test("keeps Agent logs in files without exposing a frontend log panel", () => {
  const chat = read("desktop/presentation/renderer/components/chat/LlmChat.tsx");
  const state = read("desktop/presentation/renderer/hooks/useAppState/useAppState.ts");
