...
test("私有 Provider 使用合法 KGM 样本完成真实解密并返回 MP3 产物", async () => {
  ...
  const gateway = new AuthorizedMvpProviderGateway(path.join(process.cwd(), "src", "Presentation", "worker.py"), process.cwd(), path.join(process.cwd(), ".venv", "Scripts", "python.exe"));
